...
seats = st.sidebar.selectbox("Number of Seats", [4, 5, 6, 7], help="Select the number of seats in your car.")
car_model = st.sidebar.text_input("Enter Car Model (optional)", help="Type the model name if known.")
car_year = st.sidebar.slider("Select Manufacturing Year Range", min_value=2000, max_value=2024, value=(2010, 2023), help="Choose the manufacturing year range.")
    ...
    try:
        ...
        if predicted_price is not None:
            ...
            if not filtered_comparison_df.empty:
                historical_mean_price = filtered_comparison_df["price"].mean()
                st.write(
...
filter_year = st.slider(
    "Select Year Range", min_value=int(cars_df["year"].min()), max_value=int(cars_df["year"].max()), value=(2010, 2023)
)
